...
class PaperClassification:
    ...
    def build_model(self):
        ...
        embedding_layer = tf.keras.layers.Embedding(input_dim=self.embedding_size, output_dim=self.embedding_dim, input_length=self.sentence_len)
        embedding_layer.set_weights([self.embedding_pretrained])
        Lstm_Fw = tf.keras.layers.LSTM(units=self.embedding_dim, activation='tanh', use_bias=True, unit_forget_bias=True)
        Lstm_Bw = tf.keras.layers.LSTM(units=self.embedding_dim, activation='tanh', use_bias=True, unit_forget_bias=True, go_backwards=True)
        Bilstm = tf.keras.layers.Bidirectional(layer=Lstm_Fw, backward_layer=Lstm_Bw, merge_mode='concat', input_shape=(self.batch_size, self.sentence_len))
    ...
    @staticmethod
    def data_preprocessing_v1(train: 'list', test: 'list', maxlen: 'int', max_words: 'int', lower: 'bool', filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n'):
        ...
        pad_train = tf.keras.preprocessing.sequence.pad_sequences(ind_train, maxlen=maxlen, padding='post', truncating='post')
        pad_test = tf.keras.preprocessing.sequence.pad_sequences(ind_test, maxlen=maxlen, padding='post', truncating='post')
        return Tokenlizer, pad_train, pad_test

    @staticmethod
    def data_preprocessing_v2(data: 'list', maxlen: 'int', max_words: 'int', lower: 'bool', filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n'):
        Tokenlizer = tf.keras.preprocessing.text.Tokenizer(num_words=max_words, filters=filters, lower=lower)
        Tokenlizer.fit_on_texts(data)
        ind_data = Tokenlizer.texts_to_sequences(data)
        pad_data = tf.keras.preprocessing.sequence.pad_sequences(ind_data, maxlen=maxlen, padding='post', truncating='post')
